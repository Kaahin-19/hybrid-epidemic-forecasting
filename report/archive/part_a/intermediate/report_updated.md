...
the Susceptible compartment (N4SID/S), the Infected compartment (N4SID/I), and both (N4SID/Both).
Selected global hyperparameters are n = 1 (None), n = 2 (S), n = 2 (I), and n = 2 (Both).
...
exogenous mode across all four scenarios simultaneously. For each candidate, the mean
Weighted Interval Score (WIS) is computed over every expanding window in every scenario; the
four per-scenario means are then averaged equally to give a single global score. The candidate
with the minimum global mean WIS is selected and fixed. In the second phase, the selected
fixed configuration is applied in the expanding-window forecast procedure across all four
scenarios, producing the forecast trajectories that are subsequently evaluated.
...
deviation of log Rt below 1e-8) or produced non-finite outputs at any stage, a persistence
forecast was substituted and an infinite AICc penalty was assigned to that window. This
...
windows. In multi-wave scenarios (A3 and A4), the models adapt progressively as each peak
enters the training window, maintaining reasonable alignment without major instability.
...
anticipate the new transmission level, because the models are trained only on the earlier
transmission level. The result is a systematic lag: predictions remain close to the prior level
and adjust only after enough post-transition observations accumulate in the expanding training
window. Notably, the ARX and state-space models with exogenous inputs show increased forecast
variability around the transition compared to the AR baseline. This occurs because the SIRS
compartment trajectories also shift rapidly during the intervention, causing the exogenous
covariates to encode the transition dynamics rather than predictive information about future Rt.
In this setting, the additional covariates become a source of confounding rather than signal.

Among the state-space models, N4SID and SSEST exhibit qualitatively different failure modes.
Without exogenous inputs, both select model order n = 1 and produce conservative, slowly
adapting forecasts that track long-run trends but underfit the oscillatory structure of A3 and
A4. When the Infected compartment is included as a covariate, performance improves substantially
for both families. However, N4SID/Both produces a catastrophic numerical failure on Scenario A2:
the subspace identification algorithm becomes ill-conditioned during the abrupt regime shift,
yielding forecast intervals that expand without bound and a mean WIS of approximately 5.1 × 10⁶⁵
for that scenario. The equivalent SSEST/Both configuration remains numerically stable throughout,
illustrating a systematic difference between the subspace and PEM estimation approaches under
rapid regime change.
...
windows within each scenario. Table 1 reports these values for all twelve model-exogenous
...
across 44 expanding windows. The global mean is the unweighted average across the four
scenario means. N4SID/Both is excluded from ranking because its global mean is dominated by
the numerical blow-up on A2 (mean WIS ≈ 5.1 × 10⁶⁵); per-scenario values for the remaining
three scenarios are listed for completeness.
...
‡ Catastrophic numerical blow-up; see Section 3.1.
...
AR/None achieves the lowest WIS on A2 (0.024), outperforming all ARX variants (0.043–0.044)
and most state-space configurations. Among state-space models, SSEST/I (0.030) and
N4SID/I (0.032) perform closest to AR on this scenario. This inversion arises because the
exogenous SIRS compartments encode the intervention itself: both S and I undergo rapid changes
around the transition point, and providing these as covariates to the model introduces dynamics
that describe the regime shift rather than predict future Rt. Under these conditions, the purely
...
N4SID/None (0.069) and N4SID/S (0.084) by a substantial margin. The same pattern holds for
SSEST, where SSEST/I (0.055) outperforms SSEST/None (0.050) on A1 and A2 but underperforms
...
without inputs. This suggests that exogenous mechanistic state information is most valuable
when the transmission process is evolving predictably, and becomes unreliable or counterproductive
...
0.027), but the improvement is less consistent than the analogous gain from ARX over AR.
...
Scenario A2 at the forecast stage. The subspace identification algorithm appears to become
ill-conditioned when the training data contain a rapid regime transition and the model has
...
configuration. This practical difference between the two estimation approaches is relevant to
the design of the forecasting framework in later phases of the study.
...
<!-- ============================================================
     FLAG: The following items reference figures that exist on
     disk (results/figures/) but whose specific content has not
     been visually verified in this document revision. Any
     figure caption or in-text description referring to a
     specific figure file should be confirmed against the actual
     PNG before final submission.

     Figures available on disk:
       - partA_03_forecast_plot_{SCENARIO}_{MODEL}_{EXO}.png
         (48 files, one per scenario × model-exo combination)
       - partA_04_wis_performance_boxplot.png

     Recommended figures for inclusion in the final document:
       1. partA_04_wis_performance_boxplot.png — comparative WIS
          distributions across all 12 combinations; provides a
          single-panel overview of the full results.
       2. Forecast overlay for ARX/I or ARX/Both on A1 —
          illustrates the benefit of exogenous inputs under smooth
          dynamics.
       3. Forecast overlay for AR/None on A2 — illustrates the
          regime-shift lag effect.
       4. Forecast overlay for N4SID/Both on A2 — illustrates
          the numerical blow-up (if visible in the figure).
       5. Forecast overlay for N4SID/I vs. N4SID/None on A3 or
          A4 — illustrates the benefit of the I compartment for
          state-space models on multi-wave dynamics.
     These five figures would cover the key findings without
     mechanical inclusion of all 48 combinations.
     ============================================================ -->
