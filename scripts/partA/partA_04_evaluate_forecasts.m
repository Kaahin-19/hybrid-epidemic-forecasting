%PARTA_04_EVALUATE_FORECASTS Evaluate Part A forecast artifacts.
%
%   Description:
%       Scores the final Part A forecast artifacts generated by Script 3.
%       The script reads one forecast artifact per scenario, computes WIS,
%       point-forecast errors, and interval diagnostics, then saves a compact
%       evaluation artifact and a small set of CSV tables for report use.
%
%       This script does not read Script 2 model-selection artifacts. The model
%       selection result is already embedded in the Script 3 forecast artifacts.
%
%   Workflow:
%       1. Load Part A forecast artifacts.
%       2. Score each forecast window and horizon.
%       3. Aggregate scores into report-level summaries.
%       4. Save one evaluation MAT artifact and selected CSV tables.
%
%   See also PARTA_03_RUN_FORECASTS, PARTA_CONFIG, COMPUTE_WIS, ...
...
%% 6. Local Functions
function [window_scores, horizon_scores, interval_scores] = ...
    local_evaluate_forecasts(forecast_files)
...
function [window_tbl, horizon_tbl, interval_tbl] = ...
    local_score_forecast_artifact(artifact)
...
for w = 1:num_windows
    ...
    window_rows{w} = table( ...
        ...
        window_day, window_idx, ...
        mean(wis), point.rmse, point.mae, ...
        mean(interval.coverage_mean), mean(interval.width_mean), ...
        'VariableNames', {'Scenario', 'Model', 'ExoMode', ...
        'WindowDay', 'WindowDayIdx', ...
        'WindowWIS', 'WindowRMSE', 'WindowMAE', ...
        'MeanCoverage', 'MeanIntervalWidth'});

    horizon_rows{w} = table( ...
        repmat(scenario_id, H, 1), ...
        repmat(model_type, H, 1), ...
        repmat(exo_mode, H, 1), ...
        repmat(window_day, H, 1), ...
        repmat(window_idx, H, 1), ...
        horizon_idx, forecast_day, ...
        truth_Rt, pred_Rt, ...
        point.error, point.absolute_error, point.squared_error, ...
        wis, interval.coverage_mean, interval.width_mean, ...
        'VariableNames', {'Scenario', 'Model', 'ExoMode', ...
        'WindowDay', 'WindowDayIdx', ...
        'HorizonIdx', 'ForecastDay', ...
        'Truth_Rt', 'Median_Forecast', ...
        'Error', 'AbsoluteError', 'SquaredError', ...
        'WIS', 'MeanCoverage', 'MeanIntervalWidth'});

    interval_rows{w} = table( ...
        repmat(scenario_id, K, 1), ...
        repmat(model_type, K, 1), ...
        repmat(exo_mode, K, 1), ...
        alphas, 1 - alphas, ...
        mean(interval.coverage, 1)', ...
        mean(interval.interval_width, 1)', ...
        'VariableNames', {'Scenario', 'Model', 'ExoMode', ...
        'Alpha', 'NominalCoverage', 'Coverage', 'IntervalWidth'});
end
...
summaries.scenario_summary = local_group_means(window_scores, ...
    ...
    {'WindowWIS', 'WindowRMSE', 'WindowMAE', 'MeanCoverage', 'MeanIntervalWidth'}, ...
    {'MeanWindowWIS', 'MeanWindowRMSE', 'MeanWindowMAE', ...
    'MeanCoverage', 'MeanIntervalWidth'});

summaries.horizon_summary = local_group_means(horizon_scores, ...
    {'Model', 'ExoMode', 'HorizonIdx'}, ...
    {'WIS', 'AbsoluteError', 'SquaredError', 'MeanCoverage', 'MeanIntervalWidth'}, ...
    {'MeanWIS', 'MeanAbsoluteError', 'MeanSquaredError', ...
    'MeanCoverage', 'MeanIntervalWidth'});

summaries.interval_summary = local_group_means(interval_scores, ...
    {'Model', 'ExoMode', 'Alpha', 'NominalCoverage'}, ...
    {'Coverage', 'IntervalWidth'}, ...
    {'MeanCoverage', 'MeanIntervalWidth'});
end
