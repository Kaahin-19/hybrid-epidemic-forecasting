...
%LOCAL_BUILD_REPORT Create the Markdown report text.
    ...
    if isfield(direct, 'forecast_timing') && ...
        ...
        advance_fraction = direct.forecast_timing.advance_epidemic_state_total / ...
    ...
    text = text + sprintf("- ARX epidemic-state advancement calls: %d of expected %d.\n", ...
        local_safe_count(direct, 'forecast_timing', 'advance_epidemic_state_calls'), ...
        local_safe_count(direct, 'horizon'));
    text = text + sprintf("- advance_epidemic_state share of direct ARX forecast runtime: %.1f%%.\n\n", ...
    ...
    text = text + sprintf("| advance_epidemic_state inside forecast loop | %.6f | %d |\n", ...
        direct.forecast_timing.advance_epidemic_state_total, ...
        direct.forecast_timing.advance_epidemic_state_calls);
    ...
    if advance_fraction >= 0.5
        text = text + "- The direct ARX forecast is dominated by `advance_epidemic_state`, ";
        text = text + "which indicates URDME state advancement is the next optimization target." + newline;
    else
        text = text + "- The direct ARX forecast is not dominated by `advance_epidemic_state` in this window." + newline;
    end
    ...
    text = text + "- Recommended next optimization step: profile and then reduce repeated URDME parsing/setup inside `advance_epidemic_state`; no such optimization was implemented in this pass." + newline;
end
