...
%% 3b. Selection-Stage Interval Profiling
...
%LOCAL_BUILD_INTERVAL_INPUTS Build one expanding window from the first truth.
    [truth_path, loaded, ~] = local_load_direct_truth(cfg);
    ...
    scenario_inputs = struct('Rt_true', loaded.Rt_true(:), ...
        'tspan', loaded.tspan(:), 'S_true', loaded.S_true(:), ...
        'I_true', loaded.I_true(:), 'U_true', U_true);
...
%LOCAL_BUILD_REPORT Create the Markdown report text.
    ...
    text = text + local_interval_report_section(results, ...
        "Selection-Stage State-Space Interval Profiling");
...
%LOCAL_INTERVAL_REPORT_SECTION Format interval-profiling results.
    ...
    text = text + newline;
    text = text + "- Selection-stage state-space intervals use the frozen closed-loop residual bootstrap configured by `cfg.intervals.selection_*`." + newline + newline;
end
