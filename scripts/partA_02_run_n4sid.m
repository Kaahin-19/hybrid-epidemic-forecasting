%PARTA_02_RUN_N4SID Track 3: Fast State-Space Forecasting (n4sid)
%
%   Description:
%       Executes a State-Space forecasting pipeline using the fast 
%       Subspace Identification algorithm (n4sid).
...
fprintf('Testing %d unique N4SID configurations per window.\n', num_models);
...
for i = 1:length(fileList)
    ...
    for T = windows
        ...
        parfor idx = 1:num_models
            ...
            [Rt_pred, aicc] = fit_predict_n4sid(Rt_past, U_past, U_future, n, d, horizon);
    ...
    csvName = fullfile(saveDir, sprintf('track3_model_summary_%s%s.csv', name_core, run_tag));
    writetable(summary_table, csvName);
    
    outName = fullfile(saveDir, sprintf('track3_results_%s%s.mat', name_core, run_tag));
    save_mat_atomic(outName, 'results', results, 'cfg', cfg);
    
    plot_name = sprintf('%s_N4SID%s', name_core, run_tag);
    plot_rt_forecast_comparison(results, Rt_true, tspan, plot_name, cfg);
end
fprintf('\n=== TRACK 3 Forecasts Complete ===\n');

function [Rt_curve, aicc] = fit_predict_n4sid(Rt_hist, U_hist, U_future, n, d, horizon)
    epsilon = 1e-6;
