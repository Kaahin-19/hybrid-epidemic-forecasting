%PARTA_03_EVAL_SSEST Score Track 4 (Optimized State-Space / ssest) performance.
%
%   Description:
%       Calculates and aggregates the RMSE for the ssest state-space
...
fprintf('Loading Track 4 (SSEST | %s) results from: %s\n', exo_mode, forecastDir);
files = dir(fullfile(forecastDir, ['track4_results_*', run_tag, '.mat']));

if isempty(files)
    error('EVAL:NoData', 'No Track 4 results found for exo_mode: %s.', exo_mode);
end
...
for i = 1:length(files)
    ...
    % Removes 'track4_results_' from the start and '_Both.mat' from the end
    cleanName = regexprep(filename, {'^track4_results_', [run_tag, '\.mat$']}, '');
...
%% 3. Visualization
fig = figure('Name', ['Track 4 Evaluation (', exo_mode, ')'], 'Position', [100 100 600 500]);
boxchart(scores.Scenario, scores.RMSE_Rt);
ylabel('RMSE (R_t)');
title(['Track 4: SSEST Accuracy (Exo: ', exo_mode, ')']);
grid on;

plotFile = fullfile(scoreDir, ['track4_evaluation_boxplot_', exo_mode, '.png']);
saveas(fig, plotFile);
...
fprintf('\n=== TRACK 4 (SSEST | %s): FINAL SCORECARD ===\n', upper(exo_mode));
disp(summaryTable);

fileSummary = fullfile(scoreDir, ['track4_summary_scores_', exo_mode, '.csv']);
