%PARTC_03_EVALUATE_MODELS Evaluate Part C canonical forecast artifacts.
%
%   Description:
%       Loads the frozen Part C AR/None and ARX/I forecast artifacts, computes
%       the shared Part A/B Rt evaluation metrics, and writes a cross-model
%       evaluation artifact plus named CSV tables. Figure generation is
%       intentionally delegated to Part C 05.
%
%   Workflow:
...
%       3. Build model, horizon, interval, calibration, and selection tables.
...
save(evaluation_artifact, ...
    ...
    'country_code', 'country_name', 'model_type', 'exo_mode', ...
...
%LOCAL_FORECAST_ARTIFACT_PATHS Return canonical Part C forecast artifacts.
    files = dir(fullfile(cfg.output.forecast_dir, 'partC_forecast_real_*.mat'));
    files = local_sort_dir_by_name(files);
    paths = strings(0, 1);
    for i = 1:numel(files)
        artifact_path = string(fullfile(files(i).folder, files(i).name));
        if local_forecast_path_is_active(artifact_path, cfg.fixed_forecast_cases)
            paths(end + 1, 1) = artifact_path; %#ok<AGROW>
        end
    end
end

function is_active = local_forecast_path_is_active(artifact_path, fixed_cases)
%LOCAL_FORECAST_PATH_IS_ACTIVE Check frozen Part C model identity.
    is_active = false;
    loaded = load(artifact_path, 'model_type', 'exo_mode');
...
%LOCAL_VALIDATE_FORECAST_ARTIFACT Verify required forecast fields.
    required_fields = {'experiment_id', 'experiment_name', 'data_source', ...
        ...
        'selected_configuration_artifact', 'forecast_results', ...
...
%LOCAL_SELECTION_SUMMARY_ROW Summarize fixed selected configuration use.
    ...
    row = table(string(loaded.data_source), string(loaded.country_code), ...
        ...
        'VariableNames', {'DataSource', 'CountryCode', 'CountryName', ...
        'Model', 'ExoMode', 'SelectedConfiguration', 'Source', ...
        'SelectionArtifact', 'FallbackUsed', 'ForecastArtifact'});
end
...
%LOCAL_WINDOW_ROW Build one window-level score row.
    row = table(m.DataSource, m.CountryCode, m.CountryName, m.Scenario, ...
        m.ScenarioName, m.Model, m.ExoMode, m.ForecastArtifact, ...
...
%LOCAL_EVALUATION_SETTINGS Build a one-row settings table.
    ...
    settings = table(string(cfg.experiment_id), string(cfg.experiment_name), ...
        ...
        string(cfg.input.country_name), cfg.forecast.min_window, ...
        ...
        'CountryCode', 'CountryName', 'MinWindow', 'StepSize', ...
...
%LOCAL_WRITE_TABLES Persist required Part C CSV tables.
    ...
    outputs(end + 1, 1) = local_write_table(tableDir, ...
        'partC_calibration_summary.csv', summary_tables.calibration_summary);
    outputs(end + 1, 1) = local_write_table(tableDir, ...
...
%LOCAL_REQUIRE_EXACT_CONFIGS Ensure Part C remains frozen to two cases.
    observed_configs = unique(window_scores(:, {'Model', 'ExoMode'}), 'rows');
